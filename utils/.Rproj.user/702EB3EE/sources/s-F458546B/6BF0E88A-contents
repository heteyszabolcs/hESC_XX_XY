...
input_normalization_chromhmm = function(mark = "../data/bigwig/H3K4me1_1001_pooled.hg38.scaled.bw",
...
create_edgeR_input = function(mark, bin_size = bin) {
  ...
  norm = input_normalization_chromhmm(mark = mark, input = input, bin_size = bin_size)
...
get_ranges = function(mark = "../data/bigwig/H3K4me1_1001_pooled.hg38.scaled.bw", 
                      input = "../data/bigwig/Input_1001_pooled.hg38.unscaled.bw",
                      bin_size = bin) {
  df = input_normalization_chromhmm(mark = mark,
                           input = input,
                           bin_size = bin)
  df = as.data.frame(df)
...
# save
write_tsv(input_norm_data, glue("{result_path}input_normalized_chromhmm_{as.character(bin / 1000)}kbp.tsv"))
