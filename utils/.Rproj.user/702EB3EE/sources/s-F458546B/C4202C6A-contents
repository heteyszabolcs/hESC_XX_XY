...
input_norm = "input_normalized_activegenes_10kbp.tsv"
...
diff_bind_edgeR = function(input_norm, ptm = NULL, bin_size) {
  ...
  if (is.null(ptm)) {
    write.table(
      xx_xy_fc,
      file = glue(
        "{result_folder}XX_XY_{as.character(bin_size / 1000)}kbp_mscaled_actives_fc_table.tsv"
      ),
      quote = FALSE,
      row.names = TRUE
    )
    ...
    write.table(
      xx_xy_fc,
      file = glue(
        "{result_folder}{ptm}_XX_XY_{as.character(bin_size / 1000)}kbp_mscaled_actives_fc_table.tsv"
      ),
      quote = FALSE,
      row.names = TRUE
    )
  ...
  png(
    file = glue(
      "{result_folder}XX_XY_{as.character(bin_size / 1000)}kbp_{ptm}_mscaled_actives_fc_table.png"
    ),
    width = 10,
    height = 10,
    units = 'in',
