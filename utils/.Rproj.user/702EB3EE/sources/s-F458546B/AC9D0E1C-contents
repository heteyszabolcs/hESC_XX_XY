...
# data and genotype info
subset = "chromhmm"
data = "chromhmm_10kbp.tsv"
chrom_data = "chrom_info.txt"
data = fread(glue("{result_folder}{data}"))
chrom_data = fread(glue("{data_folder}{chrom_data}"))

diff_bind_edgeR = function(data, ptm = NULL, bin_size) {
  # estimate read counts
  read_counts = data %>% mutate_at(vars(-seqnames,-start,-end,-width,-strand), ~ . * read_est)
  ...
  tmm_scaling = dge$samples
  minute_scaling = tmm_scaling %>% 
    mutate(sample = rownames(tmm_scaling)) %>%
    separate(sample, sep = ".scaled", into = c("sample")) %>% 
    inner_join(., sf, by = c("sample" = "sample_name")) %>% 
    select(group, lib.size, norm.factors = factor) %>% as.data.frame
  rownames(minute_scaling) = rownames(tmm_scaling)
  dge$samples = minute_scaling
  ...
  if (is.null(ptm)) {
    write.table(
      xx_xy_fc,
      file = glue(
        "{result_folder}XX_XY_{as.character(bin_size / 1000)}kbp_mscaled_no_inputnorm_{subset}_fc_table.tsv"
      ),
      quote = FALSE,
      row.names = TRUE
    )
    ...
    write.table(
      xx_xy_fc,
      file = glue(
        "{result_folder}{ptm}_XX_XY_{as.character(bin_size / 1000)}kbp_mscaled_no_inputnorm_{subset}_fc_table.tsv"
      ),
      quote = FALSE,
      row.names = TRUE
    )
  ...
  png(
    file = glue(
      "{result_folder}XX_XY_{as.character(bin_size / 1000)}kbp_{ptm}_mscaled_no_inputnorm_{subset}_fc_table.png"
    ),
    width = 10,
    height = 10,
    units = 'in',
...
lapply(ptms, diff_bind_edgeR, data = data, bin_size = bin_size)
